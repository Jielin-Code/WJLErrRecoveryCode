...
typedef struct
{
	...
	// 一般设置为1，2，3，4；默认设置为3，即FIRST_ERR_COMPARE_LIMIT + END_LIMIT应该大于等于START_LIMIT - 1
	int FIRST_ERR_COMPARE_LIMIT;
	// 判定当前块被纠正的参照值，一般是FIRST_ERR_COMPARE_LIMIT的基础上加上BLOCK_ERR_COMPARE_LIMIT，可以为0-8的值
	// 一般设置为1，2，3，4，5，6，7，8；默认设置为4
	int BLOCK_ERR_COMPARE_LIMIT;
...
typedef struct
{
	...
	unsigned char* InBytesArraySection; // 缓存最大可能性的字节块，共DECODER_LIST_SIZE个字节
