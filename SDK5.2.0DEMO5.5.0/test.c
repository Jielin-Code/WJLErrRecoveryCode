...
int AWGN_ERRRECOVERY(PARAMETERS* par, int type, int BlockSize, int BytesForEachBlock, double snr, int printIt, int* EncodedBytes, double* BERSum, double* BLERSum)
{
	...
	par->START_LIMIT = 12;
	par->END_LIMIT = 8;
	par->FIRST_ERR_COMPARE_LIMIT = 0;
	par->BLOCK_ERR_COMPARE_LIMIT = 0;
	par->ERRBITS_LIMIT = 8;
	par->MAXIMUM_TRAVERSAL_TIMES = 16;*/
	printf("SNR = %1.6f\n", snr);
	printf("START_LIMIT = %0d\n", par->START_LIMIT);
	printf("END_LIMIT = %0d\n", par->END_LIMIT);
	printf("ERRBITS_LIMIT = %0d\n", par->ERRBITS_LIMIT);
	printf("MAXIMUM_TRAVERSAL_TIMES = %0d\n", par->MAXIMUM_TRAVERSAL_TIMES);
	...
	if (encoder->InBytesArray == NULL || encoder->OutBytesArray == NULL || decoder->InBytesArray == NULL || decoder->OutBytesArray == NULL || decoder->BytesArray == NULL) {
		goto Err;
	}
	...
	for (i = 0; i < BlockSize; ++i) {
		...
		// 累加编码后的字节数
		*EncodedBytes = *EncodedBytes + encoder->OutBytesIndex;
	...
	*BERSum = *BERSum + ber_count;
	printf("当前帧误比特率为(The current frame bit error rate is)：ber = %1.6f\n理论误比特率(Theoretical bit error rate)：ber = %1.6f\n\n", (double)ber_count / ((double)k * 8), AWGN_BPSK_BER(snr));
	if (printIt) {
	...
	for (i = 0; i < BlockSize; ++i) {
		...
		for (j = 0; j < BytesForEachBlock; ++j) {
			if (BytesForDecode[i * BytesForEachBlock + j] != BytesForEncode[i * BytesForEachBlock + j]) {
				...
				// 这个地方必须直接跳出循环，否则bler_count会被多次计算，只要当前块出现一次纠错失败，就计算一次
				break;
			}
		}
	}
